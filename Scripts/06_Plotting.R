...
p <- ggplot() +
  ...
  geom_polygon(
    data = lake, aes(x = long, y = lat, group = group),
...
Spp <- data.frame(
  Short = c("Nighthawk", "Nightjar", "Whip-poor-will"),
...
for (i in 1:nrow(Spp)) {
  ...
  Lines.sf <- stack.df.spp %>%
    group_by(Band.Number) %>%
    summarize(do_union = F) %>%
    st_cast("LINESTRING")

  migDplot[[i]] <- ggplot(data = Continents) +
    geom_sf() +
...
fac.maps <- ggarrange(migDplot[[1]], migDplot[[2]], migDplot[[3]],
  nrow = 1, ncol = 3,
...
dens.migD.f <- ggarrange(NULL, dens.migD, dens_leg, widths = c(.1, 3, .57), nrow = 1) # f for formatted

ggarrange(fac.maps, dens.migD.f, nrow = 2, heights = c(2, 1))
ggsave("Plots/Final/Data_fig2/Mig.dist_no_legend.png",
  bg = "white", height = 6.6
)
...
top.geo.mods.chr <- map(
  .x = aictab_list3$Breeding,
  .f = \(aic.tab){
    top.geo <- aic.tab %>%
      filter(Hypothesis == "Geo") %>%
      slice_head() %>%
      pull(Modnames)
  }
)
...
elevs <- terra::extract(
  x = elev_coni, y = spp.dfs[[1]][, c("B.Long", "B.Lat")],
  ID = FALSE
) %>%
  pull()
...
plot.parms <- function(parms.df) {
  ggplot(data = parms.df, aes(
    x = Parameter, y = Beta,
    group = interaction(Species, Season), color = Species
  )) +
    geom_errorbar(aes(ymin = LCI95, ymax = UCI95),
      alpha = .3, width = 0, position = position_dodge(width = 0.75)
    ) +
    geom_errorbar(aes(ymin = LCI85, ymax = UCI85),
      alpha = .6, width = 0, size = 1, position = position_dodge(width = 0.75)
    ) +
    geom_point(position = position_dodge(width = 0.75), aes(shape = Season)) +
    ...
    theme(
      ...
      axis.text.x = element_text(size = 7, vjust = .58, angle = 60),
...
Final_mass <- plots.list[[1]] + 
  draw_image("Plots/cartoons/scale.png", x = 3.3, scale = 1)
Final_wing <- plots.list[[3]] + 
  draw_image("Plots/cartoons/Sylvia/Eastern_whip-poor-will_wing.png", x = 2.4, scale = 1)
ggarrange(Final_mass, Final_wing, nrow = 2, common.legend = TRUE, labels = "AUTO")
ggsave(paste0("Plots/Parm_ests_all_aic_less4_85CI_Br", format(Sys.Date(), "%m.%d.%y"), ".png"), bg = "white", width = 3.4)
...
form_bergs <- lapply(aictabNuis, function(df) {
  df$Modnames[1]
})
...
gen.plotdf <- function(df, form, NVs_vec) {
  ...
  ndf <- data.frame(
    B.Lat = seq(min(df$B.Lat), max(df$B.Lat), length.out = nrow(df)),
    Age = "Adult", Sex = "M"
  )
...
plot_df <- bind_rows(plot_dfs_list, .id = "SppDv") %>%
  mutate(
    Species = sapply(str_split(SppDv, "\\."), function(str) {
      str[1]
    }),
    DV = sapply(str_split(SppDv, "\\."), function(str) {
      str[2]
    })
  ) %>%
  select(-SppDv)
...
capri.fac %>%
  dplyr::select(Species, B.Lat, W.Lat) %>%
