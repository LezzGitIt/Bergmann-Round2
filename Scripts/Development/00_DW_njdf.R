...
## This script loads raw data from providers, combines data into single df, generates the time since sunset (tsss) variable, averages masses & wings from multiple captures by band age, & tidies and filters data to produce the capriBA data frame that will be used in downstream analyses

# Contents
# 1) Load & format data that will allow for seamless join into a single df
# 2) Merge into single df & continue to format
# 3) Condense age classes to Young, adult, or unknown
# 4) Calculate the difference between time since sunset & when an individual was captured
# 5) Calculate migratory distance
# 6) Create data frames for export
...
AKewpw %>% filter(!is.na(Mig.dist) & is.na(W.Lat)) # %>% View()

# Greg Various data sets
Various <- VariousGC %>%
  mutate(
    Year = sapply(str_split(Banding.Date, "/"), function(x) {
      x[3]
    }),
...
EKconi <- EKconi %>%
  mutate(
    Temp.res.mig = ifelse(Year == 2017 | Year == 2018, 7, 10),
    ...
    B.dep = as.character(as.Date(B.dep, origin = as.Date(paste0(Year, "-01-01")))),
    W.arr = as.character(as.Date(W.arr, origin = as.Date(paste0(Year, "-01-01")))),
    BandTime = ifelse(BandNumber == "1352-97758" | BandNumber == "1352-97768",
      "21:15:00", BandTime
    ),
    # Elly recommends using spring mig dist for 2 of 3 individ w/ no fall mig distance
    Dist.Fall = ifelse(!is.na(W.Lat) & is.na(Dist.Fall), Dist.Spring, Dist.Fall)
  ) %>%
...
GCeunj[c("Banding.Date", "B.dep", "W.arr")] <- lapply(GCeunj[c("Banding.Date", "B.dep", "W.arr")], function(x) {
  as.character(dmy(x))
}) # 1 failing to parse is OK

# Lars
LJeunj <- LJeunj %>% mutate(
  Species = "EUNI",
  Project = "LJdenmark",
...
njdfs_all <- lapply(njdfs_all, function(x) {
  x[, c(1:29)]
})
...
if (FALSE) {
  capri.df %>%
    filter(Project != "Greg_EDB") %>%
    select(-Project) %>%
    write.xlsx(paste0("Data/Capri_df_combined_", format(Sys.Date(), "%m.%d.%y"), ".xlsx"), row.names = FALSE, showNA = FALSE, sheetName = "Data")
}
...
capri.df <- capri.df %>%
  ...
  as.data.frame() %>%
  # Remove few individuals w/ no B.Lat or band num
  filter(!is.na(B.Lat) & !is.na(Band.Number))

capri.df <- capri.df %>%
  mutate(
    Species = ifelse(capri.df$Species == "Ceur" | capri.df$Species == "European Nightjar" | capri.df$Species == "European Nigthtjar", "EUNI", capri.df$Species),
...
capri.df$Banding.Time <- sapply(str_split(parse_date_time(capri.df[, c("Banding.Time")], c("HMS"), truncated = 3), " "), function(x) {
  x[2]
})
...
capri.df3 <- capri.df2 %>%
  filter(Age != "1" & Age != "L" & Age != "3") %>%
  mutate(
    Age = case_when(
      Age == "4" ~ "Unk", # This should be adult?
      Age == "AHY" ~ "Unk",
      Age == "ASY?" ~ "Unk",
      Age == "5" ~ "Young",
      Age == "SY" ~ "Young",
...
df <- capri.df3 %>% summarize(
  date = as.Date(Banding.Date),
  lat = B.Lat,
  lon = B.Long,
...
sun_df %>%
  arrange(format(sun_df$sunset, format = "%H:%M:%S")) %>%
  pull(sunset)
...
sun_df2 %>%
  ...
  nrow()
capri.df3 %>%
  filter(is.na(Banding.Date) | is.na(Banding.Time)) %>%
  nrow()
...
capri.df4 <- transform(capri.df4,
  Str8line =
    distHaversine(
      ...
      cbind(capri.df4$W.Long, capri.df4$W.Lat)
    ) / 1000
)
...
capri.df4 <- capri.df4 %>%
  ...
  group_by(Band.Age) %>%
  mutate(
    Wing.comb = mean(Wing.Chord, na.rm = TRUE),
    Mass.comb = mean(Mass, na.rm = TRUE)
  )
capri.df5 <- capri.df4 %>%
  filter(!is.na(Banding.Time)) %>%
...
if (FALSE) {
  capri.df5 %>%
    as.data.frame() %>%
    write.xlsx(paste0("Intermediate_products/capri.df.int_", format(Sys.Date(), "%m.%d.%y"), ".xlsx"), row.names = F)
}
